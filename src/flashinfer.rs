...
fn decode_min_chunk_pages(input: DecodePlannerPolicyInput, screened_pages: usize) -> usize {
    if matches!(screened_pages, 4 | 8 | 16)
        && input.sm == 120
        && input.batch_size == 1
...
fn screened_decode_min_chunk_pages() -> usize {
    // This environment value is intentionally compiled into each screening binary.
...
mod tests {
    use super::{
        decode_min_chunk_pages, decode_plan, decode_plan_with_screened_chunk_pages,
        decode_with_plan, screened_decode_min_chunk_pages, DecodePlannerPolicyInput,
    };
    use crate::workspace::{
    ...
    fn eligible_decode_policy_input() -> DecodePlannerPolicyInput {
        DecodePlannerPolicyInput {
            sm: 120,
            batch_size: 1,
            kv_data_type: 2,
    ...
    #[test]
    fn decode_chunk_policy_screens_only_supported_values() {
        let input = eligible_decode_policy_input();

        for candidate in [4, 8, 16] {
            assert_eq!(decode_min_chunk_pages(input, candidate), candidate);
        }
        for unsupported in [0, 2, 7, 32] {
            assert_eq!(decode_min_chunk_pages(input, unsupported), 4);
        }
    }

    #[test]
    fn decode_chunk_policy_keeps_default_outside_exact_target() {
        let eligible = eligible_decode_policy_input();
        let ineligible = [
            DecodePlannerPolicyInput {
                sm: 121,
                ..eligible
        ...
        for input in ineligible {
            assert_eq!(decode_min_chunk_pages(input, 16), 4);
        }
    }

    #[test]
    fn decode_chunk_boundaries_cover_noncontiguous_pages_once() {
        for chunk_pages in [4_usize, 8, 16] {
            let chunk_tokens = chunk_pages * 32;
            for context_len in [chunk_tokens - 1, chunk_tokens, chunk_tokens + 1] {
                let page_count = context_len.div_ceil(32);
                let physical_pages = (0..page_count).map(|page| page * 2 + 1).collect::<Vec<_>>();
                let mut visits = vec![0_u8; page_count];

                for chunk_start in (0..page_count).step_by(chunk_pages) {
                    let chunk_end = (chunk_start + chunk_pages).min(page_count);
                    for logical_page in chunk_start..chunk_end {
                        assert_eq!(physical_pages[logical_page], logical_page * 2 + 1);
                        visits[logical_page] += 1;
                    }
                }
    ...
    #[test]
    fn sm120_fp8_decode_chunk_candidates_match_control() -> Result<()> {
        ...
        let contexts = [
            31_usize, 32, 127, 128, 129, 255, 256, 257, 511, 512, 513, 1023, 1024, 2048, 4096,
        ];
        ...
        assert!(ineligible_plans[1..]
            .iter()
            .all(|candidate| candidate == &ineligible_plans[0]));
        ...
        for context_len in contexts {
            ...
            let compiled_plan = decode_plan_with_screened_chunk_pages(
                &device,
                DType::U8,
                DType::F16,
                &indptr_host,
            ...
            assert_eq!(public_plan, compiled_plan);

            let mut outputs = Vec::new();
            for chunk_pages in [4, 8, 16] {
                let plan = decode_plan_with_screened_chunk_pages(
                    &device,
                    DType::U8,
                    DType::F16,
                    &indptr_host,
            ...
            for candidate in &outputs[1..] {
                for (&control, &candidate) in outputs[0].iter().zip(candidate) {
                    let tolerance = 0.02 + 0.02 * control.abs();
                    assert!((control - candidate).abs() <= tolerance);
                }
            }
